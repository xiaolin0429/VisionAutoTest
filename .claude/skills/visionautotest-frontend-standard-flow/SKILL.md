...
description: >
  Use for ALL frontend work in VisionAutoTest involving `frontend/src` pages, components, layouts, stores, router, API modules, types, styles, or frontend docs.
  Applies to feature delivery, refactors, bug fixes, backend contract alignment, workspace-aware flows, loading and error states, and build verification in the Vue 3 MVP.
  MANDATORY TRIGGERS: Any task touching frontend/src, Vue components, Pinia stores, Element Plus UI, Tailwind styling, frontend API modules, TypeScript types for frontend,
  workspace switching UI, template canvas, test run views, or any frontend build/lint/test task in this project.
  Also trigger when the user mentions: 前端, 页面, 组件, 模板页, 执行详情, 工作空间切换, 路由, 套件, 用例列表, or any Chinese description of frontend work in VisionAutoTest.
---
