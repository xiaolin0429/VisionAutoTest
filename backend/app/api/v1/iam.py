...
from app.schemas.contracts import SessionCreate, SessionRefreshCreate, UserCreate, UserRead, UserUpdate
...
def create_session_refresh(payload: SessionRefreshCreate, request: Request, db: Session = Depends(get_db)):
    data = iam.refresh_session(db, payload.refresh_token)
    return success_response(request, data, status_code=201)


@router.get("/sessions/current")
def get_current_login_session(request: Request, current_user: User = Depends(get_current_user), current_session=Depends(get_current_session)):
    data = {
...
def delete_current_login_session(db: Session = Depends(get_db), current_session=Depends(get_current_session)):
    iam.revoke_current_session(db, current_session)
    return no_content_response()


@router.get("/users/current")
def get_current_login_user(request: Request, current_user: User = Depends(get_current_user)):
...
def list_users(
    ...
    items, total = iam.list_users(db, page=page, page_size=page_size, status=status, keyword=keyword)
    return paginated_response(request, dump_list(UserRead, items), page=page, page_size=page_size, total=total)


@router.post("/users", status_code=201)
def create_user(payload: UserCreate, request: Request, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def get_user(user_id: int, request: Request, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    user = iam.get_user(db, user_id)
    return success_response(request, dump_model(UserRead, user))


@router.patch("/users/{user_id}")
def patch_user(user_id: int, payload: UserUpdate, request: Request, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    user = iam.get_user(db, user_id)
