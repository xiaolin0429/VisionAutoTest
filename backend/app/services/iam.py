...
from app.core.security import generate_token, hash_secret, verify_secret
...
def create_session(db: Session, username: str, password: str, *, client_ip: str | None, user_agent: str | None) -> dict:
    ...
    now = utc_now()
    access_token = generate_token("vat")
    refresh_token = generate_token("rfr")
    session = UserSession(
        user_id=user.id,
        session_code=generate_token("ses"),
        access_token_jti=access_token[:64],
        client_type="web",
        client_ip=client_ip,
...
def refresh_session(db: Session, refresh_token: str) -> dict:
    ...
    new_access_token = generate_token("vat")[:64]
    new_refresh_token = generate_token("rfr")
    session.access_token_jti = new_access_token
...
def get_current_user_by_token(db: Session, token: str) -> tuple[User, UserSession]:
    session = db.scalar(
        select(UserSession).where(
            UserSession.access_token_jti == token[:64],
